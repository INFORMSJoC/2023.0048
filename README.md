...
# Optimal System Adjustment Under Operational Constraints with Applications to Infectious Disease Screening

Below, we provide a detailed description of how to use our code to generate each of the figures found in the "results" folder of this repository. Additionally, we have included all the data files that are generated by certain code files and required by others (e.g., plotting scripts in the "scripts" folder). These files may also be used for validation purposes.

## Data for Prevalence Rates Used in All Code Files
The primary calculations are performed in the Excel file "DataForPrevalenceRates", located in the "data" folder, specifically in the sheets "GRP", "Budget(1)", and "Budget(2)".
In the "Budget(2)" sheet, columns C, D, and E contain the mean, upper, and lower bounds of the prevalence rates for each disease, as referenced in the code files.
...
In the code, you set N to the desired level (25, 50, or 100). Once the code terminates, the results are stored in a variable named Yexport. 
These codes should result in data similar or close to the data in "caseStudyNumericalExperimentsminmaxregret", since the code is generating problem instances at random.
This is the only part where matlab code is required. However for the rest, python code is required.
Again this is used to generate the csv file titled "caseStudyNumericalExperimentsminmaxregret".
2) Run the code "Refined_Algo_ComputationalComplexity-Randomized" to generate the numpy data titled "data when N=25", "data when N=50", and "data when N=100". (these are not uploaded, due to their sizes, however they can be generated and stored in the same directory).
...
"CaseStudyNumericalExperimentsrandomized when N=100" used for plotting presented in the next step.
4) Run the code "PlottingComputationalComplexity_random and minmax" to generate the figure.
