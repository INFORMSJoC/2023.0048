...
def Contributionfunction(numberofviruses, mu, k, U):
    f=sum(mu[j] * math.exp(-k[j] * U[j]) for j in range(numberofviruses))
    return f
def CreateFeasibleValuesofthevectoru(levelofdescofB,number_of_viruses,B):
    ...
    allpossibledecisions1=np.hstack((np.ones(len(alluvalueswithsum)).reshape(len(alluvalueswithsum),1), alluvalueswithsum[:, :-1]))
    allpossibledecisions0=-1*np.ones(number_of_viruses+1)
    allpossibledecisions0[0]=0
    allpossibledecisions=np.vstack((allpossibledecisions0, allpossibledecisions1))
    return allpossibledecisions
...
def CreatePossibleValuesofPoverTime(timehorizon,levelofdescofP,number_of_viruses, pl, pu):
    P={}
    for t in range(timehorizon+1):
        P[t]=CreatePossibleValuesofthevectorp(levelofdescofP,number_of_viruses, pl[:, t], pu[:, t])
    return P
def CreateNodes(timehorizon,possiblepvaluesovertime,possibleuvalues,Kallowablechanges,number_of_viruses,kofviruses):
    values,actions={},{}
    alltimehorizon=np.arange(timehorizon+1)
    for t in sorted(alltimehorizon, reverse=True):
        possiblepvalues = possiblepvaluesovertime[t]
        if t==len(alltimehorizon)-1:
            for k in range(Kallowablechanges+1): # since we cant have more than Kallowablechanges.
                trials=[]
                for U in possibleuvalues[1:]:
                    valuesofU=[]
                    for p in possiblepvalues:
                        valuesofU.append(Contributionfunction(number_of_viruses,p,kofviruses,U))
                    trials.append(max(valuesofU))
                actions[str(t)+'_'+str(k)]=[0]+list(possibleuvalues[1:][np.argmin(trials)][1:])
                for p in possiblepvalues:
                    values[str(t)+'_'+str(k)+'_'+str(p)]=Contributionfunction(number_of_viruses,p,kofviruses,actions[str(t)+'_'+str(k)][1:])*k**2 #* k**2 to eusre that its best to reach the state where no more changes left, similar to having only one state at the end
        elif t==0:
            for p in possiblepvalues:
                values[str(t) + '_' + str(Kallowablechanges) + '_' + str(p)]=None
            actions[str(t) + '_' + str(Kallowablechanges)] = [0] + list(np.arange(number_of_viruses))
        else:
            for k in range(Kallowablechanges+1): # since we cant have more than Kallowablechanges.
                for p in possiblepvalues:
                    values[str(t)+'_'+str(k)+'_'+str(p)]=None
                actions[str(t)+'_'+str(k)]=[0]+list(np.arange(number_of_viruses))
        # maxK=maxK+1
    return values, actions
def ComputeaStarDeterministic(currentp, possibleu, numberofviruses,k):
    trials, feasibleu=[],[]
    for u,U in enumerate(possibleu):# ideal case can change at any point in time
        # if U[0]<=currentk:
        mu=currentp
        f=Contributionfunction(numberofviruses,mu,k,U[1:])
        # nextk=0 # not diminished as ideal case can change at every time, so K budget is not decreased
        # nextstate=str(currenttime+1)+'_'+str(nextk)+'_'+str(nextp)
        # v=values[nextstate]
        # trials.append(max(f,v))
        trials.append(f)
        feasibleu.append(U)
    minimizer=feasibleu[np.argmin(trials)]
    mini=min(trials)
    return mini, minimizer
def ComputeValueofaStateGivenanAction(action, currenttime, currentk, currentp, possiblepvaluesnextt, possibleu, values, numberofviruses,kofviruses):
    nexttime=currenttime+1
    nextk=currentk-int(action[0]) #action[0] is the y variable, i.e., change/no change
    innermaxi=[]
    mu, U = currentp, action[1:]
    f = Contributionfunction(numberofviruses, mu, kofviruses, U)
    for p in possiblepvaluesnextt:
        # Compute the optimal det action given the realization p
        objdet, actiondet = ComputeaStarDeterministic(currentp, possibleu,numberofviruses,kofviruses)
        # now fill the bellmans second part
        # nextkdet=0
        # nextstatedet=str(nexttime)+'_'+str(nextkdet)+'_'+str(p)
        nextstate=str(nexttime)+'_'+str(nextk)+'_'+str(p)
        # V=values[nextstate]-values[nextstatedet]
        F1=max(f,values[nextstate])
        # now fill the contribution functions
        # mu, U=currentp, actiondet[1:]
        # fdet=Contributionfunction(numberofviruses,mu,k,U)
        # F=f-fdet
        # F2=max(fdet,values[nextstatedet])
        # regret=F1-F2
        regret=F1-objdet
        # print(f,fdet,values[nextstate],values[nextstatedet],actiondet,action)

        # innermaxi.append(max(F,V))
        innermaxi.append(regret)
    maximizer=possiblepvaluesnextt[np.argmax(innermaxi)]
    maxi=max(innermaxi)
    # print(innermaxi)
    # print('action is {}'.format(action)+'at t {}'.format(currenttime)+' at k {}'.format(currentk)+'at p {}'.format(currentp)+'we have '+' {}'.format(maxi)+' {}'.format(maximizer))
    return maxi, maximizer
def ComputeOptimalActionValuePerKPerTime(currenttime,currentk, possibleu,values,actions, numberofviruses,kofviruses,possiblepvaluesatt, possiblepvaluesatnextt,alltimehorizon,allowablechanges):
    trials, feasibleu = [], []
    valuesforthepstates=[]
    possibleuCopy=possibleu.copy()
    for u, U in enumerate(possibleuCopy):
        if U[0]<=currentk:  # should be less than the K budget for sure, .e.g., if currentk=0, then U[0] should be 0
            trialsmax=[]
            for p in possiblepvaluesatt:
                if U[0] == 0:
                    nextk = currentk
                    nextaction = actions[str(currenttime + 1) + '_' + str(nextk)]
                    # U[1:] = nextaction[1:]
                    v, _ = ComputeValueofaStateGivenanAction([0]+list(nextaction[1:]), currenttime, currentk, p, possiblepvaluesatnextt,  possibleu, values, numberofviruses, kofviruses)
                else:
                    # if currenttime == len(alltimehorizon) - 2 and currentk == allowablechanges - 1:
                    #     neededaction=[1]+list(U[1:])
                    #     v,_=ComputeValueofaStateGivenanAction(neededaction,currenttime,currentk,p,possiblepvaluesatnextt,possibleu,values,numberofviruses,kofviruses)
                    # else:
                    v,_=ComputeValueofaStateGivenanAction(U,currenttime,currentk,p,possiblepvaluesatnextt,possibleu,values,numberofviruses,kofviruses)
                trialsmax.append(v)
            feasibleu.append(list(U))
            trials.append(max(trialsmax))
            valuesforthepstates.append(trialsmax)
    # print('at t {}'.format(currenttime)+' at k {}'.format(currentk)+' we have trials {}'.format(trials))
    if np.argmin(trials)==0: #means dont change
        minimizer=[0]+list(actions[str(currenttime + 1) + '_' + str(currentk)][1:])
    else:
        minimizer=feasibleu[np.argmin(trials)]
    actions[str(currenttime) + '_' + str(currentk)]=[z for z in minimizer]
    valuesforthepstates=valuesforthepstates[np.argmin(trials)]
    for p,P in enumerate(possiblepvaluesatt): # assign the values of the states according to the optimal action, this saves looping over the p values and call the ComputeValue function
        values[str(currenttime) + '_' + str(currentk) + '_' + str(P)] = valuesforthepstates[p]
    return values, actions
...
time_horizon=15
BT = 45
pl=np.array([HIVL,HBVU,HCVL,BabesiosisL,WNVL])
pu=np.array([HIVL,HBVL,HCVL,BabesiosisU,WNVU])
Prevalence_Data_List=np.array([Babesiosis])
# k = [0.2800,0.1600,0.1400,0.3800,0.1850]
k_used_in_viruses=[0.2800,0.1600,0.1400,0.3800,0.1850]
number_of_viruses=len(k_used_in_viruses)
levelofdescofB=10
levelofdescofP=10
allowablenbchanges=3



t1=time.time()
possible_u_values=CreateFeasibleValuesofthevectoru(levelofdescofB,number_of_viruses,BT)
possible_p_values_over_time=CreatePossibleValuesofPoverTime(time_horizon,levelofdescofP,number_of_viruses,pl,pu)
Values,Actions=CreateNodes(time_horizon,possible_p_values_over_time,possible_u_values,allowablenbchanges,number_of_viruses,k_used_in_viruses)
# maxK=2
alltimehorizon = np.arange(time_horizon + 1)
for t in sorted(alltimehorizon, reverse=True)[1:]:
    possiblepvalues_at_t = possible_p_values_over_time[t]
    possiblepvalues_at_next_t = possible_p_values_over_time[t+1]
    if t == 0:
        Values, Actions = ComputeOptimalActionValuePerKPerTime(t,allowablenbchanges,possible_u_values,Values,Actions,number_of_viruses,k_used_in_viruses,possiblepvalues_at_t,possiblepvalues_at_next_t,alltimehorizon,allowablenbchanges)
    else:
        for k in range(allowablenbchanges+1):
            Values,Actions=ComputeOptimalActionValuePerKPerTime(t,k,possible_u_values,Values,Actions,number_of_viruses,k_used_in_viruses,possiblepvalues_at_t,possiblepvalues_at_next_t,alltimehorizon,allowablenbchanges)
    print('we are in time', t)
print('time needed DP is', time.time()-t1)
...
for i in range(number_of_arcs):
    for m in range(number_of_viruses):
        ...
        PREVS[m][1] = Q[q]
    # print(PREVS)
    B, maxregret = minMaxRegret_multiple_infections(PREVS, BT, k_used_in_viruses)
...
for i in range(0, N):
    for j in range(i + 1, N):
        e1.append(i)
        e2.append(j)
optobjfunc_z2,_= Modified_BellmanFord_narrowest(e1,e2,weights,B_Regret_star_app_2,allowablenbchanges-1,source,dest,ERM_Problem,Prevalence_Data_List,k_used_in_viruses,BT)
