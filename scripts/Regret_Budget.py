...
def minMaxRegret_multiple_infections(PrevalenceRates_number_of_viruses_2, BT, k):
    ...
    # print(Possible_Prevalence_Vector_Cases)
    ...
    # print("The optimal risk values of ERM are", Risk_values_of_Possible_Cases)
    t = time.time()
...
for m in range(number_of_viruses):
    ...
    for i in range(0, N):
        sublist_upper = []
        sublist_lower = []
        sublist_upper.append(Prevalence_Data_List_Upper[m][i])
        sublist_lower.append(Prevalence_Data_List_Lower[m][i])
        for j in range(i + 1, N):
            sublist_upper.append(Prevalence_Data_List_Upper[m][j])
            sublist_lower.append(Prevalence_Data_List_Lower[m][j])
            a = min(sublist_lower)
            b = max(sublist_upper)
            A[v][p] = a
            A[v][q] = b
...
for kcounter, K_allowable_changes in enumerate(K_allowable_changes_array):
    ...
    d, path = Modified_BellmanFord_narrowest(e1,e2,weights,B_Regret_star_app_2,K_allowable_changes,source,dest,ERM_Problem,Prevalence_Data_List,k,BT)
...
import pickle
with open('F_constrained_matrix_of_ks_RegretAPP2.data', 'wb') as f:
    pickle.dump(F_constrained_matrix_of_ks, f)
f.close()
with open('solutionovertime_RegretAPP2.data', 'wb') as f:
    pickle.dump(solutionovertime, f)
f.close()
with open('group_size_path_of_ksRegretAPP2.data', 'wb') as f:
    pickle.dump(budget_path_3d_matrix_of_ks, f)
f.close()
with open('path_matrix_of_ksRegretAPP2.data', 'wb') as f:
...
# import pickle
# with open('totalcostapp2forplotting.data', 'wb') as f:
